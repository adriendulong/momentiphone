...
- (id)initWithFrame:(CGRect)frame fromViewController:(UIViewController *)bigPhotoViewController atIndex:(NSInteger)index
{
    ...
    if (self) {
        // Initialization code
        self.bigPhotoViewController = bigPhotoViewController;
...
- (void)show
{
	...
	[self.bigPhotoViewController.view addSubview:self];
...
- (UIButton *)doneButton
{
	if (!_doneButton) {
		...
		[_doneButton setTitle:NSLocalizedString(@"Terminé", @"Title for Done button") forState:UIControlStateNormal];
		...
		[_doneButton.titleLabel setFont:[UIFont boldSystemFontOfSize:13.0f]];
		_doneButton.alpha = 0.;
...
{
	if ([_delegate respondsToSelector:@selector(photoBrowser:didTapOnDoneButton:atIndex:)]) {
		self.displayingDetailedView = NO;
		[_delegate photoBrowser:self didTapOnDoneButton:sender atIndex:_currentlySelectedIndex];
	}
}
