...
- (void)setPullToRefreshActionHandler:(void (^)(void))actionHandler {
    ...
    titleLabel.text = NSLocalizedString(@"Tirer pour actualiser...",);
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"Dernière mise à jour: %@",), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:NSLocalizedString(@"Jamais",)];
}

- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
	dateFormatter = newDateFormatter;
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"Dernière mise à jour: %@",), self.lastUpdatedDate?[newDateFormatter stringFromDate:self.lastUpdatedDate]:NSLocalizedString(@"Jamais",)];
}
    ...
    if(pullToRefreshActionHandler) {
        switch (newState) {
            ...
            case SVPullToRefreshStateTriggered:
                titleLabel.text = NSLocalizedString(@"Relâcher pour actualiser...",);
                [self rotateArrow:M_PI hide:NO];
                break;
                
            case SVPullToRefreshStateLoading:
                titleLabel.text = NSLocalizedString(@"Mise à jour...",);
